...
if [ "$#" -eq 0 ]; then
    APP_DIR="$(cd "$(dirname "$0")/.." >/dev/null 2>&1; pwd -P)/packages/zuraffa_inappwebview"
...
for pkg in "zuraffa_inappwebview" "zuraffa_inappwebview_platform" "zuraffa_inappwebview_android" "zuraffa_inappwebview_ios" "zuraffa_inappwebview_macos"; do
    pubspec="packages/$pkg/pubspec.yaml"
    sed -i.bak -E "s/^version: .*/version: $VERSION/" "$pubspec" && rm -f "$pubspec.bak"
    sed -i.bak -E "/^( *zuraffa_inappwebview: \^)/s|.*|  zuraffa_inappwebview: ^$VERSION|" "$pubspec" && rm -f "$pubspec.bak"
    sed -i.bak -E "/^( *zuraffa_inappwebview_platform: \^)/s|.*|  zuraffa_inappwebview_platform: ^$VERSION|" "$pubspec" && rm -f "$pubspec.bak"
