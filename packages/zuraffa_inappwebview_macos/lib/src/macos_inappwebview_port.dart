...
/// macOS [InappwebviewPort] over the typed
/// [MacosInappwebviewChannel].
class MacosInappwebviewPort implements InappwebviewPort {
  final MacosInappwebviewChannel channel;

  const MacosInappwebviewPort({required this.channel});
  ...
  Future<List<InappwebviewValue>> invoke({
    ...
    if (values is! List) {
      throw const MacosInappwebviewException(
        'malformed_response',
