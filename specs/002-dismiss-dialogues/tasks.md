...
- [x] T002 `[TDD]` (US2, D3, D4, D5) `DialogueDismissScript.source` — canonical script removes fixed/sticky, resets overflow/margin, top-level only — NEW `packages/zuraffa_inappwebview/lib/src/dialogue_dismiss.dart`
- [x] T003 `[TDD]` (US3, D6) `DialogueDismissPolicy` — attempts min 1 default 1, delay default zero — same file

## Phase 2: Service facade

- [x] T004 `[TDD]` (US2, D7, D8, D9) `WebviewService.dismissDialogues({id, policy})` evaluates the script once by default; `not_created` typed failure for unknown ids; port errors swallowed — `packages/zuraffa_inappwebview/lib/src/webview_service.dart`
