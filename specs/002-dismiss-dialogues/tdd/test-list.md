...
| D3 | canonical script hides `position: fixed`/`sticky` elements (source contains computed-position filter + reversible `display: none`) | T002 | DONE |
...
| D11 | an `Error` (not only an `Exception`) raised during dismissal is swallowed too | T004 | DONE |
