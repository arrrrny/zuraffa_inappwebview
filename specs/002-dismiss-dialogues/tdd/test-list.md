...
| D3 | canonical script removes `position: fixed`/`sticky` elements — executed under a JS engine against a fixed/sticky/static fixture: exactly the two overlay nodes are removed | T002 | DONE |
| D4 | canonical script resets `overflow`/`margin` on `documentElement` + `body` — same executed fixture: roots that start dirty come back cleared | T002 | DONE |
